...
import org.apache.spark.shuffle.sort.CometShuffleExternalSorter.SpillSorter;
...
public class CometCelebornShuffleExternalSorter {
    private static final Logger logger = LoggerFactory.getLogger(CometCelebornShuffleExternalSorter.class);
    ...
    private final ConcurrentLinkedQueue<CelebornSpillSorter> spillingSorters = new ConcurrentLinkedQueue<>();
    ...
    public CometCelebornShuffleExternalSorter(int numPartitions, TaskContext taskContext, StructType schema,
            CometShuffleMemoryAllocatorTrait allocator, ShuffleWriteMetricsReporter writeMetrics, int initialSize,
    ...
    class CelebornSpillSorter extends CelebornSpillWriter {
        ...
        public void writeSortedNative(boolean tracingEnabled) {
            ...
            ShuffleInMemorySorter.ShuffleSorterIterator sortedRecords = new ShuffleInMemorySorter.ShuffleSorterIterator(
                    pos, this.sorterArray, 0);
            ...
            while (sortedRecords.hasNext()) {
                ...
                if (partition != currentPartition) {
                    if (partition != -1) {
                        long written = doSpilling(dataTypes, rowPartition, writeMetrics, preferDictionaryRatio, compressionCodec, compressionLevel, tracingEnabled);
            ...
            synchronized (writeMetrics) {
              writeMetrics.incRecordsWritten(
                  ((ShuffleWriteMetrics) writeMetrics).recordsWritten());
              taskContext
                  .taskMetrics()
        ...
        /**
         * Reset the in-memory sorter's pointer array only after freeing up the memory
         * pages holding the
         * records.
         */
        public void reset() {
    ...
    /**
     * Checks whether there is enough space to insert an additional record in to the
     * sort pointer
     * array and grows the array if additional space is required. If the required
     * space cannot be
    ...
    /**
     * Close the sorter, causing any buffered data to be sorted and written out to
     * disk.
     *
     * @return metadata for the spill files written by this sorter. If no records
     *         were ever inserted
