 ...
 */
public class CometCelebornUnsafeShuffleWriter<K, V> extends ShuffleWriter<K, V> {
    ...
    @Nullable
    private CometCelebornShuffleExternalSorter sorter;
     ...
     * success = true and
     * then call stop() with success = false if they get an exception, we want to
     * make sure we don't
     * try deleting files, etc twice.
     */
    private boolean stopping = false;

    CometCelebornUnsafeShuffleWriter(TaskMemoryManager memoryManager, BaseShuffleHandle<K, V, V> handle, long mapId,
            TaskContext taskContext, SparkConf conf, CelebornConf celebornConf,
            ShuffleWriteMetricsReporter writeMetrics, ShuffleClient shuffleClient, Long celebornShuffleId) {
    ...
    private void open() {
        ...
        assert (sorter == null);
        allocator = CometShuffleMemoryAllocator.getInstance(
                sparkConf,
                memoryManager,
                Math.min(
                        CometShuffleExternalSorter.MAXIMUM_PAGE_SIZE_BYTES, memoryManager.pageSizeBytes()));
        sorter = new CometCelebornShuffleExternalSorter(partitioner.numPartitions(), taskContext, schema, allocator,
                writeMetrics,
    ...
    private void closeAndWriteOutput() {
        assert (sorter != null);
    ...
    private void insertRecordIntoSorter(Product2<K, V> record) {
        assert (sorter != null);
